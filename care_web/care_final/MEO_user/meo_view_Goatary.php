    ...
    <section class="content">
      ...
      <?php
        ...
        if(!empty($village)){
          // 
          // 
          $get_detail="SELECT * FROM `care_master_mtf_livestock_tarina_meo` WHERE `livestock`='1'  and `care_LS_vlg_name`='$village' AND `care_LS_month`='$months' AND `care_LS_year`='$Year' and `care_LS_MEO_status`='1' ";
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          if (($sql_get_detail_fetch['care_LS_CBO_comment_status']==1)&& ($sql_get_detail_fetch['care_LS_mt_comment_status']==1)) {
                            ?>
                               <a class="btn btn-primary btn-xs" href="MEO_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('9')?>&care_hhi=<?=web_encryptIt($sql_get_detail_fetch['care_LS_slno'])?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" ><u>View</u></a>
                               <a class="btn btn-danger btn-xs" href="MEO_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('9')?>&care_hhi=<?=web_encryptIt($sql_get_detail_fetch['care_LS_slno'])?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=3&reset=<?=web_encryptIt($sql_get_detail_fetch['care_serial_id'])?>" onclick="return confirm('Are you sure you want to delete this item?');"><u>Delete</u></a>
                               <!-- care_serial_id -->
                            <?php
                          }else{
                            echo "--";
