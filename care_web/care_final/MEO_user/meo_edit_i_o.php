    ...
    <section class="content">
      ...
      <?php
        ...
        if(!empty($village)){
          // 
          // 
          $get_detail="SELECT * FROM `care_master_input_output_tracking_tarina` WHERE
          `care_TARINA_vlg_name`='$village' AND `care_TARINA_month`='$months' AND `care_TARINA_year`='$Year' and `care_IP_OP_MEO_status`='0'";
                   ...
                   <table id="example" class="display" cellspacing="0" width="100%" style="">
                    ...
                    <tfoot>
                     ...
                     <tbody>
                      ...
                      while ($sql_get_detail_fetch=mysqli_fetch_assoc($sql_get_detail)) {
                        $xi++;
                        //`care_TARINA_slno`, `care_TARINA_year`, `care_TARINA_month`, `care_TARINA_district_name`, `care_TARINA_block_name`, `care_TARINA_gp_name`, `care_TARINA_vlg_name`, `care_TARINA_hhi`, `care_TARINA_hhi_slno`, `care_TARINA_w_farmer_name`, `care_TARINA_spouse_name`, `care_TARINA_event_date`, `care_TARINA_activity_name`, `care_TARINA_support_provide`, `care_TARINA_production`, `care_TARINA_consumption`, `care_TARINA_sale`, `care_TARINA_remarks`, `care_TARINA_participant_sign`, `care_TARINA_shg_name`, `care_TARINA_social_caste`, `care_TARINA_status`, `care_TARINA_entry_date`, `care_TARINA_entry_time`, `care_TARINA_long_id`, `care_TARINA_lat_id`, `care_TARINA_employee_id`, `care_IP_OP_mt_comment_empty`, `care_IP_OP_mt_comment_date`, `care_IP_OP_mt_comment_time`, `care_IP_OP_mt_id`, `care_IP_OP_mt_status`, `care_IP_OP_CBO_comment_empty`, `care_IP_OP_CBO_comment_date`, `care_IP_OP_CBO_comment_time`, `care_IP_OP_CBO_comment_status`, `care_IP_OP_CBO_id`, `care_IP_OP_MEO_status`, `care_IP_OP_MEO_date`, `care_IP_OP_MEO_time`, `care_IP_OP_MEO_id`
                        ?>
                      <tr>
                        ...
                        <td>
                          ...
                          if (($sql_get_detail_fetch['care_IP_OP_mt_status']==1)&& ($sql_get_detail_fetch['care_IP_OP_CBO_comment_status']==1)) {
                            ?>
                               <a class="btn btn-primary btn-xs" href="MEO_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('12')?>&care_hhi=<?=web_encryptIt($sql_get_detail_fetch['care_TARINA_slno'])?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=1" ><u>Edit</u></a>
                            <?php
                          }else{
                            echo "--";
